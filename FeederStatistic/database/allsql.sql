...
--adagolótípus frissítése(0=Siemens S,F,HS; 1=Siemens X ; 2=Fuji ; 3=RLI ; 4=VCD & SEQ):
UPDATE repair SET r_type = 0 WHERE ds7i = "544544";
select * from repair where ds7i="544544";

--adagoló méret módosítása -feeder_size tábla alapján!
select * from repair where ds7i="265326";
UPDATE repair SET size = 3 WHERE ds7i = "291690";

--operator frissites:
select * from repair where ds7i="265326";
UPDATE repair SET wd_num = "82832",op_name = "Paska István",line = 9 WHERE id = 6280;
...
users.id = u_id and u_id = 13 and 
r_date >= "2013-02-18" and r_date <= "2013-10-17" 
order by r_date;
...
--Arhivalashoz....
delete from repair where r_date <= "2012-12-31";
delete from feeder_errors where r_id < 6560;
delete from feeder_works where r_id < 6560;
delete from usedparts where r_id < 6560;
...
--Aktuális sorról kieset adagolók (sorok.id alapján : 0=H,1=I,2=J,3=L stb.) :
...
and line = 3;
--Sorokról kiesett adagolók ds7i-ként összesítve :
...
ORDER BY sor,datum;
--Ki melyik soron mennyit rakott ki és mikor (név szerint összehasonlítva!):
SELECT dolgozok.name as operator,sorok.name as sor, repair.ds7i, repair.r_date as datum 
FROM repair,sorok,dolgozok 
WHERE (sorok.id = line) and (dolgozok.name = repair.op_name) AND (r_date >= "2012-09-01" AND r_date <= "2012-09-31") 
ORDER BY sor,datum;
--Ki melyik soron mennyit rakott ki név szerint :
...
--Legtöbbet visszahozott feederek (összesített) TOP 50  + TÍPUS + MÉRET:
