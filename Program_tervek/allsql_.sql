...
--delete from feeder_tpm where tpm_outdate <= "2018-12-31";
...
JOIN sorok ON fej.felh_sor_id = sorok.id 
JOIN gepek ON fej.felh_gep_id = gepek.id 
WHERE hely <> 1 AND (ki_oka = 1 OR ki_oka = 3) AND ki_datum >= "2019-01-01" AND ki_datum <= "2019-12-30" 
ORDER BY ki_datum;
...
--fej típusok:
SELECT * FROM fej WHERE hely <> 1 GROUP BY sorozatszam ORDER BY sorozatszam;

--egyéb alkatrészek:
SELECT egyeb_alkatreszek.*, alkatreszek.megnevezes, alkatreszek.rendelesi_szam FROM egyeb_alkatreszek 
JOIN alkatreszek ON alkatreszek.id = egyeb_alkatreszek.alkatresz_id
ORDER BY egyeb_alkatreszek.szekreny_szama;

--osszes alkatresz:
select * from alkatreszek where torolve=0 group by rendelesi_szam order by megnevezes;




--D-ODC adatbázis:
SELECT * FROM rossz_parameter WHERE rParameterNev = "sTranszportSeb" AND rJovahagyva = 0 AND rRosszErtek = 12;

SELECT setupsheets.*, sorok.srNev FROM setupsheets, sorok WHERE sAktiv = 1 AND sorok.srid = sor_id ORDER BY sid;


--ois_report:
delete from ois_infok;